...
class CustomTextDataset(Dataset):
    ...
    '''
    def __init__(self, train_file_path, transform=None, target_transform=None):
    ...
    def back_to_text(self, idx):

       return self.tokenizer.decode(encoded_input["input_ids"][idx])
    ...
    def __getitem__(self, idx):
        ...
        inputs = {
            'prompt':self.bert_tokenizer(self.data.iloc[idx]['prompt']),
            'model_a': self.data.iloc[idx]['model_a'],
            'model_b': self.data.iloc[idx]['model_b'],
            'response_a': self.bert_tokenizer(self.data.iloc[idx]['response_a']),
            'response_b': self.bert_tokenizer(self.data.iloc[idx]['response_b'])
        }
